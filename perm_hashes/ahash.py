    ...
    for i in range(0, height - block_size + 1, block_size):
        for j in range(0, width - block_size + 1, block_size):
            ...
            if block.shape == (block_size, block_size):
                ...
                position_array = np.array([i, j])
                flattened_block = transformed_block.flatten()
                # Concatenate block position as fixed-size array
                flattened_block_with_position = np.concatenate((flattened_block, position_array))
                reshaped_block = flattened_block_with_position[:block_size*block_size].reshape((block_size, block_size))
                block_hash = imagehash.average_hash(Image.fromarray(reshaped_block.astype(np.uint8)), hash_size=block_size)
...
# Save transformed images for verification
Image.fromarray(transformed_original_image_array).save('transformed_original_image.jpeg')
Image.fromarray(transformed_tampered_image_array).save('transformed_tampered_image.jpeg')
