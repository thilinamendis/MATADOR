    ...
    for i in range(0, height, block_size):
        for j in range(0, width, block_size):
            ...
            if block.shape == (block_size, block_size):
                ...
                resized_block = resize_block(transformed_block, dhash_size)
                block_hash = imagehash.dhash(Image.fromarray(resized_block), hash_size=block_size)
...
# Calculate dHashes for transformed images
...
# Display results
print(f"Original Perceptual Hash (dHash): {original_perceptual_hash}")
print(f"Tampered Perceptual Hash (dHash): {tampered_perceptual_hash}")
...
print(f"Transformed Original Perceptual Hash (dHash): {transformed_original_perceptual_hash}")
print(f"Transformed Tampered Perceptual Hash (dHash): {transformed_tampered_perceptual_hash}")
