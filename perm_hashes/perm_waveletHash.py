...
def apply_magic_square(image, magic_square):
    ...
    hash_size = 8  # Use 8x8 size for whash
    height, width = image.shape
    ...
    for i in range(0, height, block_size):
        for j in range(0, width, block_size):
            ...
            if block.shape == (block_size, block_size):
                ...
                resized_block = resize_block(transformed_block, hash_size)
                block_hash = imagehash.whash(Image.fromarray(resized_block), hash_size=hash_size)
...
# Calculate waveletHashes for transformed images
...
# Display results
print(f"Original Perceptual Hash (waveletHash): {original_perceptual_hash}")
print(f"Tampered Perceptual Hash (waveletHash): {tampered_perceptual_hash}")
...
print(f"Transformed Original Perceptual Hash (waveletHash): {transformed_original_perceptual_hash}")
print(f"Transformed Tampered Perceptual Hash (waveletHash): {transformed_tampered_perceptual_hash}")
