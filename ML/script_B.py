...
# Usage
# python3 Person_B/script_B.py Person_B/images/ Person_B/config/config.json Person_B/hashes
...
def divide_into_blocks(image, block_size):
    ...
    for i in range(0, image.shape[0], block_size):
        for j in range(0, image.shape[1], block_size):
            block = image[i:i+block_size, j:j+block_size] # Corrected indexing
            blocks.append(block)
...
# Main function to handle user inputs and verify the images
def main():
    ...
    start_num = config["start_num"]  # Read start_num from the config
    ...
    for image_filename in image_files:
        ...
        # If the image is tampered, compare path proofs
        if is_tampered:
            ...
            # Compare recalculated hashes with the original hashes
            start_time = time.time()
            tampered_blocks = []
            ...
            # Save the recalculated hashes for comparison
            start_time = time.time()
